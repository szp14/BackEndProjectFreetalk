...
class TKuser(models.Model):
	...
	def newResp(self, respType, content, imgList, attachment, postId, respId):
		...
		q = TKresponse(respType = respType, content = content, img = img, attachment = attachment, user = self.user, post = p, respId = respId, hostId = hostId)
		q.save()
		for img in imgList:
	...
	def upvoteResp(self, respId):
		if TKupvoteRelation.isUpvoted(1, self.id, postId):
			return False
	...
	def downvoteResp(self, respId):
		if TKupvoteRelation.isUpvoted(1, self.id, postId) == False:
			return False
		q = TKresponse.objects.filter(id = respId)
		if q:
			q = q[0]
...
class TKclassTag(models.Model):
	...
	def modifyClassTag(self, newTag):
		newTag.strip()
		if TKclassTag.searchPostByClassTag(self.classTagName):
			return False
		posts = TKhomepage.searchPostByClassTag(self.classTagName)
		for p in posts:
			p.classTag = ' '.join([tag for tag in p.classTag.split() if tag != self.classTagName])
